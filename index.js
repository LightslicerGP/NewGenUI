...
const types = [
  {
    "4 Tabs with Action Button":
      ...
      <div class="tab-bar glassy">
        <div class="tab-item" id="selected">
...
function showCurrentType() {
  ...
  let html = typeObj[key];

  html = html.replace(/(\sclass="[^"]*)\bselected\b([^"]*")/g, (m, pre, post) =>
    pre + post.replace(/selected/g, '').replace(/ +/g, ' ').replace(/"$/, '')
    + ' id="selected"\"'
  );
  html = html.replace(/class="tab-item selected"/g, 'class="tab-item" id="selected"');
...
document.getElementById("test-playground-btn").addEventListener("click", () => {
  window.location.href = "TestPlayground";
});